...
import { Loader2, Plus, Trash2, Users, Film, Video, Download, AlertCircle } from 'lucide-react';
    ...
    const handleGenerateVeoVideo = async () => {
        ...
        try {
            ...
            if (uri) {
                ...
                if (!vidResp.ok) throw new Error("Gagal mengunduh data video.");
                const blob = await vidResp.blob();
                const blobUrl = URL.createObjectURL(blob);
                setVideoUrl(blobUrl);
            } else {
                throw new Error("Video berhasil dibuat tetapi URL tidak ditemukan.");
            }
                ...
                <div className="p-6 bg-gray-900 rounded-2xl shadow-xl border border-gray-700 flex flex-col h-full">
                    ...
                    <div className="mt-6">
                        ...
                        <p className="text-xs text-gray-500 mt-2 text-center">
                            *Menggunakan prompt bahasa Inggris di sebelah kiri. Pastikan prompt sudah final.
                        </p>
                    </div>
                </div>
            </div>
